...
import { Clock, Percent, ShoppingCart, ArrowRight, Tag, Flame } from 'lucide-react';
...
export default function DealsPage() {
  ...
  const handleAddToCart = async (product: Product) => {
    ...
    const finalPrice = parseFloat(product.price) * (1 - product.discount_percent / 100);
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 relative z-10 w-full">
        ...
        <div className="text-center mb-16 space-y-4">
          ...
          <p className="font-rajdhani text-gray-400 max-w-xl mx-auto font-semibold tracking-wider text-base sm:text-lg">
            Kultowe tytuły w wyjątkowo obniżonych cenach. Oferty zmieniają się co 24 godziny! Złap je, zanim znikną.
          </p>
        </div>

        {/* Dynamic Countdown Timer Widget */}
        <div className="max-w-xl mx-auto mb-16">
          <div className={`backdrop-blur-md bg-black/60 border rounded-2xl p-6 md:p-8 flex flex-col items-center justify-center text-center shadow-lg transition-all duration-500 ${
            isUnderOneHour 
            ...
            <div className="flex items-center gap-4 md:gap-6 font-orbitron text-3xl sm:text-5xl font-black tracking-widest select-none">
              <div className="flex flex-col items-center">
                <span className={isUnderOneHour ? 'text-neonPink text-glow-pink' : 'text-white'}>
                  {formatTime(timeLeft.hours)}
                </span>
                <span className="font-rajdhani text-xs font-bold text-gray-500 tracking-wider mt-1">GODZIN</span>
              </div>
              ...
              <div className="flex flex-col items-center">
                ...
                <span className="font-rajdhani text-xs font-bold text-gray-500 tracking-wider mt-1">MINUT</span>
              </div>
              <span className="text-gray-600 self-start mt-1">:</span>
              <div className="flex flex-col items-center">
                <span className={isUnderOneHour ? 'text-neonPink text-glow-pink animate-pulse' : 'text-neonOrange text-glow-pink'}>
                  {formatTime(timeLeft.seconds)}
                </span>
                <span className="font-rajdhani text-xs font-bold text-gray-500 tracking-wider mt-1">SEKUND</span>
              </div>
            </div>
        ...
        {loading ? (
          ...
          <div className="backdrop-blur-md bg-black/40 border border-white/10 rounded-2xl p-16 text-center shadow-lg space-y-6">
            ...
            <p className="font-rajdhani text-gray-400 max-w-sm mx-auto font-semibold">
              W tym momencie wszystkie gry są sprzedawane w regularnych cenach. Wróć tu wkrótce!
            </p>
            <Link
              href="/store"
              ...
              return (
                ...
                >
                  ...
                  <div className="relative aspect-video w-full overflow-hidden bg-black/40">
                    <Image
                      src={failedImages[product.id] ? product.image_url : `https://images.unsplash.com/photo-1607604276583-eef5d076aa5f?w=600&auto=format&fit=crop&q=80&text=${encodeURIComponent(product.name)}`}
                      alt={product.name}
                      fill
                    ...
                    <div className="mt-auto flex items-end justify-between">
                      <div className="flex flex-col">
                        ...
                        <span className="font-orbitron font-extrabold text-2xl text-white text-glow-pink">
                          {discountPrice.toFixed(2)} <span className="text-sm font-bold">PLN</span>
                        </span>
                      </div>
